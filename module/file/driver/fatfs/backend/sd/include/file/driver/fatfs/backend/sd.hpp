...
namespace file::driver::fatfs::backend::sd
{
	struct Card_info
	{
		enum Type
	...
	class SD_media_interface : public Media_interface
	{
	  public:

		SD_media_interface() = default;

		DSTATUS disk_initialize() override { return sd::disk_initialize(0); }
		DSTATUS disk_status() override { return sd::disk_status(0); }
		DRESULT disk_read(BYTE* buff, LBA_t sector, UINT count) override { return sd::disk_read(0, buff, sector, count); }
		DRESULT disk_write(const BYTE* buff, LBA_t sector, UINT count) override
		{
			...
			return sd::disk_write(0, buff, sector, count);
		}
		DRESULT disk_ioctl(BYTE cmd, void* buff) override { return sd::disk_ioctl(0, cmd, buff); }
	};

	inline void mount_media_handler()
	{
		file::driver::fatfs::media_interface = std::make_unique<SD_media_interface>();
	}
}