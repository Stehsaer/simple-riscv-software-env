...
namespace device::qemu::virtio
{
	struct IO
	{
		inline static constexpr uint32_t expected_magic_value = 0x74726976;
		...
		// Check Magic Number
		inline bool check_magic_value() const { return magic_value == expected_magic_value; }

		// Select Queue Index as the current queue
		inline void select_queue(uint32_t queue_idx) { queue_sel = queue_idx; }

		// Inform the device of the size of the current queue
		inline void set_queue_size(uint32_t queue_size) { queue_num = queue_size; }

		// Notify the device about a queue
		inline void notify_queue(uint32_t queue_idx) { queue_notify = queue_idx; }

		// Confirm that the queue is ready
		inline void confirm_queue_ready() { queue_ready = 1; }

		// Check if the device is a valid Virtio block device
		inline bool is_block_device() const { return check_magic_value() && version == 2 && device_id == 2; }
	...
	struct Block_request
	{
	  ...
	  public:
		...
		inline static Block_request write(uint64_t sector) { return {Request_type::Write, sector}; }
		inline static Block_request read(uint64_t sector) { return {Request_type::Read, sector}; }
		inline static Block_request flush() { return {Request_type::Flush, 0}; }
	};
	...
	struct Interface
	{
		...
		inline size_t size() const { return descriptors.size(); }
	};
	...
	class Block_device_interface
	{
		...
		static inline constexpr size_t default_queue_size = 4;
