...
namespace driver::fat32
{
	...
	std::pair<error_t, std::unique_ptr<os::filesystem::File_descriptor>> Device::open(
	...
	error_t Device::Fd::fstat(struct stat& info) noexcept
	{
		return ENOTSUP;
	}
...
extern "C"
{
	...
	{
		if (driver::fat32::media_interface == nullptr) return STA_NOINIT;
		return driver::fat32::media_interface->disk_initialize();
	}

	DSTATUS disk_status(BYTE pdrv)
	{
		if (driver::fat32::media_interface == nullptr) return STA_NOINIT;
		return driver::fat32::media_interface->disk_status();
	}

	DRESULT disk_read(BYTE pdrv, BYTE* buff, LBA_t sector, UINT count)
	{
		if (driver::fat32::media_interface == nullptr) return RES_NOTRDY;
		return driver::fat32::media_interface->disk_read(buff, sector, count);
	}

	DRESULT disk_write(BYTE pdrv, const BYTE* buff, LBA_t sector, UINT count)
	{
		if (driver::fat32::media_interface == nullptr) return RES_NOTRDY;
		return driver::fat32::media_interface->disk_write(buff, sector, count);
	}

	DRESULT disk_ioctl(BYTE pdrv, BYTE cmd, void* buff)
	{
		if (driver::fat32::media_interface == nullptr) return RES_NOTRDY;
		return driver::fat32::media_interface->disk_ioctl(cmd, buff);
	}
}