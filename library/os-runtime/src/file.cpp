...
#pragma region Filesystem_interface
	...
	{
		if (fd < 0 || fd >= file_descriptor_map.size()) return nullptr;
	...
	Fd_t Filesystem_interface::dup(Fd_t oldfd)
	{
		...
		if (avail_fd >= file_descriptor_map.size()) file_descriptor_map.resize(avail_fd + 1, nullptr);
	...
	Fd_t Filesystem_interface::dup2(Fd_t oldfd, Fd_t newfd)
	{
		...
		if (newfd >= file_descriptor_map.size()) file_descriptor_map.resize(newfd + 1, nullptr);
