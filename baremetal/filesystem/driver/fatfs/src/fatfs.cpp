...
namespace filesystem::driver::fatfs
{
	...
	static error_t convert_fat32_errcode(
		FRESULT result,
	...
	std::pair<error_t, std::unique_ptr<filesystem::File_descriptor>> Device::open(
...
extern "C"
{
	...
	{
		if (filesystem::driver::fatfs::media_interface == nullptr) return STA_NOINIT;
		return filesystem::driver::fatfs::media_interface->disk_initialize();
	}

	DSTATUS disk_status(BYTE pdrv [[maybe_unused]])
	{
		if (filesystem::driver::fatfs::media_interface == nullptr) return STA_NOINIT;
		return filesystem::driver::fatfs::media_interface->disk_status();
	}

	DRESULT disk_read(BYTE pdrv [[maybe_unused]], BYTE* buff, LBA_t sector, UINT count)
	{
		if (filesystem::driver::fatfs::media_interface == nullptr) return RES_NOTRDY;
		return filesystem::driver::fatfs::media_interface->disk_read(buff, sector, count);
	}

	DRESULT disk_write(BYTE pdrv [[maybe_unused]], const BYTE* buff, LBA_t sector, UINT count)
	{
		if (filesystem::driver::fatfs::media_interface == nullptr) return RES_NOTRDY;
		return filesystem::driver::fatfs::media_interface->disk_write(buff, sector, count);
	}

	DRESULT disk_ioctl(BYTE pdrv [[maybe_unused]], BYTE cmd, void* buff)
	{
		if (filesystem::driver::fatfs::media_interface == nullptr) return RES_NOTRDY;
		return filesystem::driver::fatfs::media_interface->disk_ioctl(cmd, buff);
	}
}
