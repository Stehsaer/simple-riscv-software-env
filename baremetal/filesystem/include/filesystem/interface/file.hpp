...
namespace filesystem
{
	...
	struct Device
	{
		...
		virtual std::pair<error_t, std::unique_ptr<File_descriptor>> open(
	...
	class Interface
	{
		...
		// Get device from id.
		// -- Return `nullptr` if not found.
		Device* get_device(Dev_id_t dev_id);
	  ...
	  public:
		...
		Interface()
		{
			empty_fd = std::make_unique<File_descriptor_container>(
				std::make_unique<Empty_fd>(),
				INT_MAX,
				0,
				false,
				false
		...
		error_t mount_device(Dev_id_t& dev_id, std::string_view path, std::unique_ptr<Device> device);
		error_t mount_device(std::string_view path, std::unique_ptr<Device> device);
