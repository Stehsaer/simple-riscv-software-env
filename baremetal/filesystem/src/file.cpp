...
#pragma region Interface
	...
	Dev_id_t Interface::get_available_dev_id() const
	{
	...
	Fd_t Interface::get_available_fd() const
	{
	...
	Interface::File_descriptor_container* Interface::get_file_descriptor(Fd_t fd)
	{
	...
	Device* Interface::get_device(Dev_id_t dev_id)
	{
	...
	Dev_id_t Interface::get_device_id(std::string_view name)
	{
	...
	error_t Interface::mount_device(Dev_id_t& dev_id, std::string_view path, std::unique_ptr<Device> device)
	{
	...
	error_t Interface::mount_device(std::string_view path, std::unique_ptr<Device> device)
	{
		const util::Scoped_lock lock;
	...
	error_t Interface::unmount_device(std::string_view path)
	{
		const util::Scoped_lock lock;
	...
	off_t Interface::lseek(Fd_t fd, off_t offset, int whence)
	{
		const util::Scoped_lock lock;
	...
	ssize_t Interface::read(Fd_t fd, void* buf, size_t count)
	{
		const util::Scoped_lock lock;
	...
	ssize_t Interface::write(Fd_t fd, const void* buf, size_t count)
	{
		const util::Scoped_lock lock;
	...
	error_t Interface::fstat(Fd_t fd, struct stat& info)
	{
		const util::Scoped_lock lock;
	...
	error_t Interface::close(Fd_t fd)
	{
		const util::Scoped_lock lock;
	...
	int Interface::ftruncate(Fd_t fd, off_t length)
	{
		const util::Scoped_lock lock;
	...
	Fd_t Interface::dup2(Fd_t oldfd, Fd_t newfd)
	{
		const util::Scoped_lock lock;
	...
	int Interface::open(Fd_t& fd, std::string_view path, int flags, mode_t mode)
	{
		const util::Scoped_lock lock;
	...
	error_t Interface::stat(std::string_view path, struct stat& info)
	{
		const util::Scoped_lock lock;
	...
	error_t Interface::unlink(std::string_view path)
	{
		const util::Scoped_lock lock;
	...
	error_t Interface::link(std::string_view oldname, std::string_view newname)
	{
		const util::Scoped_lock lock;
